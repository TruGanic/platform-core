...
/**
 * Recursively sort object keys for canonical JSON (must match client signedClient.js)
 */
...
export function createSignaturePayload(request: {
  ...
  // Filter out excluded headers (case-insensitive); use lowercase keys to match client
  ...
  // Canonical payload: sort keys so payload string matches client (client uses sortObjectKeys)
  const payload = {
    method: request.method,
    path: request.path,
    timestamp: request.timestamp,
    nonce: request.nonce,
    body: sortObjectKeys(normalizedBody),
    headers: sortObjectKeys(otherHeaders),
  };
