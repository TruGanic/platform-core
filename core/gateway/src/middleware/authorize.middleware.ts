...
/**
 * Local authorization middleware (for backward compatibility)
 * Checks permissions locally without calling Security service
 *
 * Use this if you want to avoid the extra HTTP call
 * But you lose centralized policy management and audit logging
 *
 * Usage:
 *   app.post('/api/servers/:serverId', authorizeLocalMiddleware('write:servers'), handler)
 */
export function authorizeLocalMiddleware(requiredPermission: string) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!did) {
      return res.status(401).json({
        error: "Unauthenticated - DID not found",
      });
    }
    ...
    if (!hasPermission) {
      log.warn("Local authorization denied", {
        did,
        requiredPermission,
